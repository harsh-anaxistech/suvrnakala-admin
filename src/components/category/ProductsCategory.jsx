...
const ProductCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        const response = await API.put(`api/productscategories/${editCategoryId}`, { name: categoryName });
        setCategories(prev => prev.map(cat => cat._id === editCategoryId ? response.data : cat));
        toast.success('Category updated successfully');
      } else {
        const response = await API.post('api/productscategories', { name: categoryName });
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      setIsLoading(true);
      try {
        await API.delete(`api/productscategories/${id}`);
  ...
  return (
    <Container fluid className="px-4 py-4">
      ...
      <Card className="border rounded-0">
        <Card.Body>
          <Card.Title className="mb-3">All Product Categories</Card.Title>
